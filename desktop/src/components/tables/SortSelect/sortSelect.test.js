...
import SortSelectHeadHOC,  { SortSelectHead } from 'components/tables/SortSelect/head';
import SortSelectToolbarHOC,  { SortSelectToolbar } from 'components/tables/SortSelect/tool';
...
describe(`Sort Select Table Component`, () => {
  ...
  it(`should create appropriate sort handler`, () => {
    ...
    val = instance.desc({ 'asdf': { 'qwer': 1 } },{ 'asdf': { 'qwer': 0 } },`asdf`,TableDataTypes.OBJECT,[`qwer`]);     
    expect(val).toEqual(-1);
    val = instance.desc({ 'asdf': { 'qwer': -1 } },{ 'asdf': { 'qwer': 0 } },`asdf`,TableDataTypes.OBJECT,[`qwer`]);   
    expect(val).toEqual(1);
    val = instance.desc({ 'asdf': { 'qwer': 0 } },{ 'asdf': { 'qwer': 0 } },`asdf`,TableDataTypes.OBJECT,[`qwer`]);    
    expect(val).toEqual(0);
  });
  it(`stable sort`, () => {
    const wrapper = setup();   
    const instance = wrapper.instance();    
    let array = [{ 'asdf': 1 },{ 'asdf': 1  },{ 'asdf': 0 }];
    let sorter = instance.getSorting(`asc`,`asdf`,`asdf`,null);
  ...
  it(`should return sort based on "desc" or "asc" by this.getSorting`, () => {
    const wrapper = setup();   
    const instance = wrapper.instance();    
    let val = instance.getSorting(`asc`,`asdf`,`asdf`,null)({ 'asdf':  -1  },{ 'asdf':  0  });
    expect(val).toEqual(-1);
    val = instance.getSorting(`desc`,`asdf`,`asdf`,null)({ 'asdf': -1 },{ 'asdf':  0  });
    expect(val).toEqual(1);
  });
  ...
  it(`should call this.handleClick on row clickage`, () => {
    const wrapper = setup();
    wrapper.find(`#row-0`).last().simulate(`click`);  
  });
});
...
const setupHeadHOC = () => {
  return  mount(<table><SortSelectHeadHOC {...headProps} /></table>);
};
...
describe(`Sort Select Head Component`, () => {
  ...
  it(`should create appropriate sort handler`, () => {
    ...
    const instance = wrapper.find(`SortSelectHead`).first().instance();
    ...
    expect(headProps.onRequestSort).toHaveBeenCalledWith({ event: `asdf` },`a`,`b`,`c`);
  });
});
