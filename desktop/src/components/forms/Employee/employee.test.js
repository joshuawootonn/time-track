...
import {
  Employee,
  EMPLOYEE_FORM_RESET_BUTTON_ID,
  EMPLOYEE_FORM_CLEAR_BUTTON_ID
} from 'components/forms/Employee/employee';
...
describe(`Employee Component`, () => {
  ...
  it(`should call props.resetForm on #${EMPLOYEE_FORM_RESET_BUTTON_ID}`, () => {
    ...
    wrapper
      .find(`#${EMPLOYEE_FORM_RESET_BUTTON_ID}`)
      .first()
      .simulate(`click`);
  ...
  it(`should only show the clear button when props.type === filter`, () => {
    ...
    expect(
      wrapperWithCorrectType.find(`#${EMPLOYEE_FORM_CLEAR_BUTTON_ID}`).length
    ).toBe(1);
    const wrapperWithIncorrectType = setup();
    expect(
      wrapperWithIncorrectType.find(`#${EMPLOYEE_FORM_CLEAR_BUTTON_ID}`).length
    ).toBe(0);
  });
  it(`should call props.resetForm and props.clearFilter on #${EMPLOYEE_FORM_CLEAR_BUTTON_ID}`, () => {
    ...
    wrapper
      .find(`#${EMPLOYEE_FORM_CLEAR_BUTTON_ID}`)
      .first()
      .simulate(`click`);
    expect(props.resetForm).toHaveBeenCalledTimes(1);
