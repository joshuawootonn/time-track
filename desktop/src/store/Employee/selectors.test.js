...
describe(`Employee Selectors`, () => {
  ...
  test(`getAllEmployees should return a mapped version of results for valid entities and results`, () => {
    const returnedValue=employeeSelectors.getAllEmployees.resultFunc({ 1: { val: `asdf` } },[1]);
    expect(returnedValue).toEqual([{ val:`asdf` }]);
  });
  test(`getSelectedEmployee should return {} for when analyze.employee -1  `, () => {
    const returnedValue = employeeSelectors.getSelectedEmployee.resultFunc({},analyzeInitialState);
    expect(returnedValue).toEqual({});
  });
  test(`getSelectedEmployee should return selected authority when`, () => {
    const returnedValue = employeeSelectors.getSelectedEmployee.resultFunc({ 1: { value: `asdf` } },{ ...analyzeInitialState,employee:1 });
    expect(returnedValue).toEqual({ value:`asdf` }); 
  });
  test(`getSelectedEmployee should return {} for when analyze.employee -1  `, () => {
    const returnedValue = employeeSelectors.getCurrentEmployee.resultFunc({},employeeInitialState);
    expect(returnedValue).toEqual({});
  });
  test(`getSelectedEmployee should return selected authority when`, () => {
