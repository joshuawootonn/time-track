    ...
    if(filters) {
      list = list.filter(project => {
        ...
        Object.keys(filters).forEach(key => {
          if(key === `name` && filters[key] !== `` && !((new RegExp(`${filters[key]}`,`i`)).test(`${project[key]}`))){
            decision = false;
          }
          if(key === `startTime` &&  moment(project[`date`]).isBefore(moment(filters[key],`MM-DD-YY HH:mm:ss`))){
            decision = false;
          }
          if(key === `endTime` && moment(project[`date`]).isAfter(moment(filters[key],`MM-DD-YY HH:mm:ss`))){
            decision = false;
          }
