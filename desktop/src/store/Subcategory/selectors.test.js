...
describe(`Subcategory Selectors`, () => {
  test(`should have two basic selectors`, () => {
    subcategorySelectors.getSubcategoriesFromEntities({
      entities: { subcategories: {} }
    });
    subcategorySelectors.getSubcategoriesFromResults({
      results: { subcategories: [] }
    });
  });
  test(`getAllSubcategories should return null for results.size === 0`, () => {
    let returnedValue = subcategorySelectors.getAllSubcategories.resultFunc(
      {},
      null
    );
  ...
  test(`getAllSubcategories should return a mapped version of results for valid entities and results`, () => {
    const returnedValue = subcategorySelectors.getAllSubcategories.resultFunc(
      { 1: { val: `asdf` } },
      [1]
    );
  ...
  test(`getSelectedSubcategory should return {} for when analyze.authority -1  `, () => {
    const returnedValue = subcategorySelectors.getSelectedSubcategory.resultFunc(
      {},
      initialState
    );
