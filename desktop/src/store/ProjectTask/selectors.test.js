...
describe(`Subcategory Selectors`, () => {
  test(`should have two basic selectors`, () => {
    projectTaskSelectors.getProjectTasksFromEntities({
      entities: { projectTasks: {} }
    });
    projectTaskSelectors.getProjectTasksFromResults({
  ...
  test(`getAllSubcategories should return a mapped version of results for valid entities and results`, () => {
    const returnedValue = projectTaskSelectors.getAllProjectTasks.resultFunc(
      { 1: { val: `asdf`, projectId: 1, taskId: 1 } },
      [1],
      { 1: { val: `zxcv` } },
      { 1: { val: `qwer` } }
    );
  ...
  test(`getAllProjectTasksObjects should return object of task with content on valid content`, () => {
    ...
    returnedValue = projectTaskSelectors.getAllProjectTasksObjects.resultFunc(
      returnedValue
    );
    expect(returnedValue).toEqual({
      4: {
  ...
  test(`getSelectedProject should return {} for when analyze.authority -1  `, () => {
    const projectEntities = projectSelectors.getProjectsFromEntities({
      entities: { projects: {} }
    });
    const allProjectTasks = projectTaskSelectors.getAllProjectTasks.resultFunc(
      { 1: { val: `asdf`, projectId: 1, taskId: 1 } },
      [1],
      { 1: { val: `zxcv` } },
      { 1: { val: `qwer` } }
    );
    const returnedValue = projectTaskSelectors.getSelectedProject.resultFunc(
      projectEntities,
      initialState,
      allProjectTasks
    );
  ...
  test(`getSelectedProject should return selected authority when`, () => {
    const projectEntities = projectSelectors.getProjectsFromEntities({
      entities: { projects: { 1: { id: 1, val: `asdf` } } }
    });
    const allProjectTasks = projectTaskSelectors.getAllProjectTasks.resultFunc(
      { 1: { val: `asdf`, projectId: 1, taskId: 1 } },
      [1],
