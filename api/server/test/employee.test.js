...
describe('/employee', () => {
  ...
  test('gets all employee', done => {
    return request(app).get('/api/employees').expect(200).then(response => {
      expect(response.body.length).toBe(2);
      done();
    })
  })
  test('gets a employee', done => {
    return request(app).get('/api/employees/1').expect(200).then(response => {
      expect(response.body.firstName).toBe('Joshua');
      done();
    })
  })
  test('creates a new employee', (done) => {
    return request(app).post('/api/employees').send(createData).expect(200, done);
  });
  test('deletes a employee', (done)=> {
    return request(app).delete('/api/employees/3').send().expect(200,done);
  })
  test('updates employee 3', done => {
    return request(app).put('/api/employees/2').send(updateData).expect(200,done);
  })
