    ...
    ui(new Ui::MainWindow)
{
    ...
    QObject::connect(&idle_timer,SIGNAL(timeout()),this,SLOT(when_idle_for_30()));


    QObject::connect(filter,SIGNAL(active()),this,SLOT(when_active()));


    input = new Input_Dialog;
    QObject::connect(input,SIGNAL(hidden()),this,SLOT(close_input_dialog()));
    QObject::connect(input,SIGNAL(finished(int)),this,SLOT(close_input_dialog()));
    QObject::connect(input,SIGNAL(inactive()),this,SLOT(loginInitialize()));
...
void MainWindow::ConnectEmployees()
{
 ...
 */
void MainWindow::BroadCast(QString x)
{
    ui->statusBar->showMessage(x);
    Sleep(1000);
    ui->statusBar->showMessage("");
}
void MainWindow::open_input_dialog(int page_number)
{
    idle_timer.stop();
    input->show();
    input->setPage(page_number);
    this->setEnabled(false);

}
void MainWindow::close_input_dialog()
{
...
void MainWindow::when_idle_for_30(){

    open_input_dialog(3);

}
void MainWindow::when_active(){


    if(ui->stackedWidget->currentIndex()!=0&&!input->isVisible())
    {
        qDebug()<<"wow active in main";
...
/* Section: Login
 * Purpose: This section deals with the login page
 */
void MainWindow::loginInitialize()
{
...
void MainWindow::admin_initialize()
{
    ...
    ui->admin_clockin5hoursago->hide();
    ui->admin_reset->hide();
...
void MainWindow::admin_update_employee_list()
{
    ...
    ui->admin_employee_list_table->hideColumn(3);
    ui->admin_employee_list_table->hideColumn(4);
    ...
    ui->admin_employee_list_table->setHorizontalHeaderItem(2,new QTableWidgetItem("Admin"));
...
void MainWindow::admin_update_job_list()
{
    ...
    ui->admin_project_list_table->hideColumn(1);
    ui->admin_project_list_table->hideColumn(2);
    ui->admin_project_list_table->hideColumn(3);
...
void MainWindow::employee_initialize()
{
    ...
    ui->employee_slider->setValue(10);
    employee_update_tables(10);
    idle_timer.start();

}
void MainWindow::employee_update_tables(int pos)
{
    ...
    if(now_date.dayOfWeek()==7)
    {
        start.setDate(now_date.addDays(0));
        finish.setDate(now_date.addDays(6));
    }
    else
    {
        start.setDate(now_date.addDays(-now_date.dayOfWeek()));
        finish.setDate(now_date.addDays(6-now_date.dayOfWeek()));
    }

    int multiplier=10-pos;
    if(pos!=10)
    {
        int x = -7*multiplier;
        //qDebug()<<x;
        start.setDate(start.date().addDays(x));
        finish.setDate(finish.date().addDays(x));
        //qDebug()
    ...
    if(pos==10)
    {
        ui->employee_time_label->setText("This Week");
    }
    else if(pos== 9)
    {
        ui->employee_time_label->setText("Last Week");
    }
    else
    {

        //qDebug()<<"im here";
        ui->employee_time_label->setText("From: "+start.date().toString("MMMM dd, yyyy")+"             To: " +finish.date().toString("MMMM dd, yyyy"));
    }


    employee_update_employee(start,finish);
}
void MainWindow::employee_update_employee(QDateTime start, QDateTime finish)
{
    QString x = start.toString("yyyy-MM-dd HH:mm:ss");
    QString y = finish.toString("yyyy-MM-dd HH:mm:ss");
    //qDebug()<<selected_id;
    QSqlQuery qry(employeeDataBase);
    ui->employee_employee_table->clearContents();
    ...
    if(qry.exec())
    {
        while(qry.next())
        {
            ...
            c = date1.toString("ddd, MMM d");//DateSyntax("date",date1);
...
void MainWindow::employeePage_initialize()
{
    ...
    ui->employee_page_box1->hide();
    ui->employee_page_box2->hide();
...
void MainWindow::update_employee_page_table1()
{
    ...
    ui->Employee_page_table1->setSortingEnabled(true);
    QSqlQuery qry(employeeDataBase);
    qry.prepare("SELECT name,pin,adminstatus,wage,id FROM employeelist");
    QString a,b,c,d,e;
    ...
    ui->Employee_page_table1->hideColumn(3);
    ui->Employee_page_table1->hideColumn(4);
    ...
    ui->Employee_page_table1->setHorizontalHeaderItem(2,new QTableWidgetItem("Admin"));
...
{
    ui->Employee_page_table2->setSortingEnabled(true);
    //qDebug()<<selected_id<<x<<y;
    QSqlQuery qry(employeeDataBase);
    ui->Employee_page_table2->clearContents();
    ...
    ui->Employee_page_table2->hideColumn(5);
    ui->Employee_page_table2->setHorizontalHeaderItem(0,new QTableWidgetItem("Date"));
    ui->Employee_page_table2->setHorizontalHeaderItem(1,new QTableWidgetItem("In"));
    ui->Employee_page_table2->setHorizontalHeaderItem(2,new QTableWidgetItem("Out"));
    ui->Employee_page_table2->setHorizontalHeaderItem(3,new QTableWidgetItem("Length"));
    ui->Employee_page_table2->setHorizontalHeaderItem(4,new QTableWidgetItem("Lunch"));
...
void MainWindow::projectPage_initialize()
{
    ...
    ui->blah->hide();
    ui->blah2->hide();
    ui->project_p_combo->hide();
    ui->project_p_combo2->hide();
    ui->project_p_combo3->hide();
    update_projectPage_tables();
...
{
    ui->Project_page_table1->setSortingEnabled(true);
    QSqlQuery qry(projectDataBase);
    QString active ="1";
    qry.prepare("SELECT name,id FROM projectlist where active='"+active+"'");
    QString a,b;
    ...
    ui->Project_page_table1->hideColumn(1);
    ui->Project_page_table1->hideColumn(2);
    ui->Project_page_table1->hideColumn(3);
    ...
    ui->Project_page_table1->setEditTriggers(QAbstractItemView::DoubleClicked);
...
void MainWindow::update_project_page_table2()
{
    ...
    ui->Project_page_table2->setSortingEnabled(true);
    ...
    project_update_project_combo();
    project_update_project_combo2();
    project_update_project_combo3();

}
void MainWindow::project_update_project_combo()
{
    QSqlQueryModel * modal=new QSqlQueryModel();
    QSqlQuery* qry=new QSqlQuery(projectDataBase);
    ...
    qry->exec();
    modal->setQuery(*qry);
    ui->project_p_combo->setModel(modal);

}
void MainWindow::project_update_project_combo2()
{
    QString x = ui->project_p_combo->currentText();
    QSqlQueryModel * modal2=new QSqlQueryModel();
    QSqlQuery* qry2=new QSqlQuery(projectDataBase);
    qry2->prepare("select DISTINCT sub from itemlist where cate='"+x+"'");
    qry2->exec();
    modal2->setQuery(*qry2);
    ui->project_p_combo2->setModel(modal2);

}
void MainWindow::project_update_project_combo3()
{
    QString x = ui->project_p_combo2->currentText();
    QSqlQueryModel * modal3=new QSqlQueryModel();
    QSqlQuery* qry2=new QSqlQuery(projectDataBase);
    qry2->prepare("select dim from itemlist where sub='"+x+"'");
    qry2->exec();
    modal3->setQuery(*qry2);
    ui->project_p_combo3->setModel(modal3);
...
void MainWindow::on_login_edit_editingFinished()
{
    ...
    if (qry1.exec())
    {
        ...
        if(count > 1)
        {
            ...
            ui->login_label_status->setText("Duplicate password. Contact admin.");
        }
        if(count < 1)
        {

            if(ui->login_edit->text()=="")
                ui->login_label_status->setText("Please enter your pin number.");
            else
                ui->login_label_status->setText("Invalid pin number. Try again.");
        }

    }

}
void MainWindow::on_login_enter_button_clicked()
{
    pin=ui->login_edit->text();
...
void MainWindow::on_admin_employee_list_table_pressed()
{

    employeePage_initialize();
}
void MainWindow::on_admin_project_list_table_pressed()
{
...
void MainWindow::on_employee_slider_sliderMoved(int position)
{
    qDebug()<<position;


    employee_update_tables(position);
}
...
void MainWindow::on_Employee_page_table1_cellPressed(int row, int column)
{
    ...
    ui->Employee_page_table2->setRowCount(0);
...
void MainWindow::on_employee_add_employee_clicked()
{
    ...
    name = ui->employee_page_line_edit_name->text();
    ID = ui->employee_page_line_edit_pin->text();
    if(ui->employee_page_check_as->isChecked())
        adminstatus="1";
    else
        adminstatus="0";
    wage = ui->employee_page_line_edit_wage->text();
    qry.clear();
    ...
    ui->employee_page_check_as->setChecked(false);
    ui->employee_page_line_edit_name->setText("");
    ui->employee_page_line_edit_pin->setText("");
    ui->employee_page_line_edit_wage->setText("");
}
...
void MainWindow::ShowContextMenuEmployee1(const QPoint& pos)
{
    ...
    QMenu myMenu;
    myMenu.setFont(ui->stackedWidget->font());
    ...
    if(selectedAction)
    {
        ...
        else if(selectedAction->text()=="Delete")
        {
            ...
            update_employeePage_tables();
            if(error==0)
                BroadCast("Delete Success!");
            else
                BroadCast("Error with the Delete function!");
        ...
        {
            for(int i = 0; i<ui->Employee_page_table1->rowCount();i++)
            {
...
void MainWindow::ShowContextMenuEmployee2(const QPoint& pos)
{
    ...
    myMenu.addAction("Delete");
    QSqlQuery qry(employeeDataBase);
    qry.prepare("SELECT clockout FROM '"+selected_id+"' WHERE shiftnumber='"+selected_shiftnumber+"'");
    if(qry.exec())
    {
        QString x;
        while(qry.next())
             x= qry.value(0).toString();
        if(x=="")
            myMenu.addAction("Clockout and Edit");
        else
    ...
    if(selectedAction)
    {
        ...
        else if(selectedAction->text()=="Clockout and Edit")
        {
            QSqlQuery qry(employeeDataBase);
            QDateTime unformatted_clockout = QDateTime::currentDateTime();
            QDateTime clock = format_datetimes(unformatted_clockout);
            QString time = clock.toString("yyyy-MM-dd HH:mm:ss");



            QString na = "Na";
            qry.prepare("update '"+selected_id+"' set clockout='"+time+"',lunch='0',one = '"+na+"',two = '"+na+"',three = '"+na+"',four = '"+na+"',five = '"+na+"'"
               ",six = '"+na+"',seven = '"+na+"',eight = '"+na+"',nine = '"+na+"',ten = '"+na+"',eleven = '"+na+"',twelve = '"+na+"'  where shiftnumber = '"+selected_shiftnumber+"'");
            qry.exec();
            qry.clear();



            qry.prepare("update employeelist set active=0 where id = '"+selected_id+"'");
            qry.exec();
            qry.clear();
 ...
 */
void MainWindow::on_Project_page_table1_cellPressed(int row, int column)
{
    column = column;
    ui->Project_page_table2->clear();
    ui->Project_page_table2->setRowCount(0);
    QString lul = ui->Project_page_table1->item(row,0)->text();
    QSqlQuery qry(projectDataBase);
    qry.prepare("SELECT id FROM projectlist WHERE name = '"+lul+"'");
    if(qry.exec())
    {
        while(qry.next())
        {
...
void MainWindow::on_Project_page_table2_cellPressed(int row, int column)
{
    column = column;
    QString lul = ui->Project_page_table2->item(row,0)->text();
    QSqlQuery qry(projectDataBase);
    qry.prepare("SELECT id FROM '"+selected_pl_id+"' WHERE name = '"+lul+"'");
    if(qry.exec())
    {
        while(qry.next())
        {
           selected_item = qry.value(0).toString();
        }
    }
    //qDebug()<<lul<<selected_pl_id<<selected_item;
}
void MainWindow::on_project_add_project_clicked()
{
    ...
    QString name = ui->project_line_edit->text();
    ...
    ui->project_line_edit->setText("");
}
void MainWindow::on_project_add_item_clicked()
{
    QString cate = ui->project_p_combo->currentText();
    QString sub = ui->project_p_combo2->currentText();
    QString dim = ui->project_p_combo3->currentText();
    QSqlQuery qry(projectDataBase), qry1(projectDataBase);

    QString x = QString::number(ui->Project_spin1->value())+":00";
    QString y = QString::number(ui->Project_spin2->value())+".0";
    QString composite;
    int error=0;
    if(sub=="Na"&&dim=="Na")
    {
    ...
    project_update_project_combo2();
    project_update_project_combo3();
    ui->Project_spin1->setValue(0);
    ui->Project_spin2->setValue(0);
}
void MainWindow::on_project_p_combo_currentIndexChanged(const QString &arg1)
{
    project_update_project_combo2();
    QString x = arg1;
}
void MainWindow::on_project_p_combo2_currentIndexChanged(const QString &arg1)
{
    project_update_project_combo3();
    QString x = arg1;
}
...
void MainWindow::ShowContextMenuProject1(const QPoint& pos)
{
    ...
    if(selectedAction)
    {
        ...
        {
            for(int i = 0; i<ui->Project_page_table1->rowCount();i++)
            {
                QString name = ui->Project_page_table1->item(i,0)->text();
                QString ID = ui->Project_page_table1->item(i,1)->text();
...
void MainWindow::ShowContextMenuProject2(const QPoint& pos)
{
    ...
    if(selectedAction)
    {
        ...
        {
            QSqlQuery qry(projectDataBase);
            qry.prepare("DELETE FROM '"+selected_pl_id+"' WHERE id='"+selected_item+"'");
            qry.exec();
            update_projectPage_tables();
            BroadCast("Delete Success!");
        }
    ...
    else if (minutes==0&&hours==0)
    {
        ...
        if(ui->shiftedit_checkBox1->checkState()==2)
        {
            ...
            if(projectQry.exec())
            {
                ...
                qDebug()<<"Problem with the first query in shiftedit checkbox 1";
            }
            QString item1id;
            qDebug()<<project1id<<item1;
            ...
            if(itemQry.exec())
            {
                ...
                qDebug()<<"Problem with the second query in shiftedit checkbox 1";
            }
